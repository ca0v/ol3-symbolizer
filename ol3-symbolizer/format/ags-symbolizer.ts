 ...
 */
import * as Symbolizer from "./ol3-symbolizer";
...
export class StyleConverter {
  ...
  private asColor(
    color:
      | ArcGisFeatureServerLayer.Color
      | undefined
  ) {
  ...
  private fromSFSSolid(
    symbol: ArcGisFeatureServerLayer.Symbol,
    style: Format.Style
  ) {
    style.fill = {
  ...
  private fromSFS(
    ...
    switch (symbol.style) {
      ...
      case "esriSFSForwardDiagonal":
        this.fromSFSForwardDiagonal(
          symbol,
          style
        );
        break;
      case "esriSFSBackwardDiagonal":
        this.fromSFSBackwardDiagonal(
          symbol,
          style
        );
        break;
  ...
  private fromPMS(
    ...
    style.image["anchor-x"] =
      this.asWidth(symbol.xoffset || 0);
    style.image["anchor-y"] =
      this.asWidth(symbol.yoffset || 0);
    style.image.imgSize = [
      this.asWidth(symbol.width),
      this.asWidth(symbol.height),
    ];
  }

  private fromSLSSolid(
    symbol: ArcGisFeatureServerLayer.Outline,
    style: Format.Style
  ) {
    if (!symbol.width)
      throw "symbol width required";
  ...
  private fromSLS(
    symbol:
      | ArcGisFeatureServerLayer.Outline
      | undefined,
    style: Format.Style
  ) {
    if (!symbol)
  ...
  private fromPFS(
    ...
    style.fill = {
      image: {
        ...
        "anchor-x": this.asWidth(
          symbol.xoffset || 0
        ),
        "anchor-y": this.asWidth(
          symbol.yoffset || 0
        ),
        imgSize: [
          this.asWidth(symbol.width),
    ...
    switch (renderer.type) {
      ...
      case "classBreaks": {
        ...
        let classBreakRenderer = <
          ArcGisFeatureServerLayer.ClassBreakRenderer
        >renderer;
        if (
          classBreakRenderer.classBreakInfos
        ) {
          console.log(
            "processing classBreakInfos"
          );
          if (
            classBreakRenderer.visualVariables
          ) {
            classBreakRenderer.visualVariables.forEach(
              (vars) => {
                switch (vars.type) {
                  /**
                  ...
                  case "sizeInfo": {
                    ...
                    if (
                      ...
                      classBreakRenderer.classBreakInfos.forEach(
                        (
                          classBreakInfo
                        ) => {
                          let icons =
                            steps.map(
                              (
                                ...
                                let json =
                                  <
                                    ArcGisFeatureServerLayer.Symbol
                                  >JSON.parse(
                                    JSON.stringify(
