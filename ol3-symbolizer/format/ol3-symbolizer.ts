import { IConverter } from "./@types/base";
...
export class StyleConverter
  implements IConverter<Format.Style>
{
  ...
  private serializeStyle(
    ...
    if (style.getAnchor) {
      ...
      "anchorXUnits,anchorYUnits,anchorOrigin"
        .split(",")
        .forEach((k) => {
    ...
    if (style.path) {
      ...
      if (style.fill)
        assign(s, "fill", style.fill);
      if (style.scale)
        assign(s, "scale", style.scale); // getScale and getImgSize are modified in deserializer
      if (style.imgSize)
        assign(
          s,
          "imgSize",
  ...
  private serializeFill(fill: Fill) {
    if (!fill) return null;
  ...
  private deserializeStyle(
    ...
    if (json.circle)
      image = this.deserializeCircle(
        json.circle
      );
    else if (json.star)
      image = this.deserializeStar(
        json.star
      );
    else if (json.icon)
      image = this.deserializeIcon(
        json.icon
      );
    else if (json.svg)
      image = this.deserializeSvg(
        json.svg
      );
    else if (
      json.image &&
    ...
    if (json.text)
      text = this.deserializeText(
        json.text
      );
    if (json.fill)
      fill = this.deserializeFill(
        json.fill
      );
    if (json.stroke)
      stroke = this.deserializeStroke(
        json.stroke
      );
  ...
  private deserializeText(
    ...
    return new Text(<any>{
      fill:
        json.fill &&
        this.deserializeFill(json.fill),
      stroke:
        json.stroke &&
        this.deserializeStroke(
          json.stroke
        ),
      text: json.text,
      font: json.font,
  ...
  private deserializeStar(
    ...
    let image = new RegularShape(<any>{
  ...
  private deserializeIcon(
    ...
    let image = new Icon(<any>{
      anchor: json.anchor || [0.5, 0.5],
      anchorOrigin:
        json.anchorOrigin || "top-left",
      anchorXUnits:
        json.anchorXUnits || "fraction",
      anchorYUnits:
        json.anchorYUnits || "fraction",
      //crossOrigin?: string;
      img: undefined,
      imgSize: undefined,
      offset: json.offset,
  ...
  private deserializeSvg(
    ...
    let icon = new Icon(<any>{
      ...
      anchorXUnits:
        json.anchorXUnits || "pixels",
      anchorYUnits:
        json.anchorYUnits || "pixels",
      //crossOrigin?: string;
      offset: json.offset,
      offsetOrigin: json.offsetOrigin,
      opacity: json.opacity,
  ...
  private deserializeStroke(json: any) {
    ...
    doif(json.lineDash, (v) =>
      stroke.setLineDash(v)
    );
    doif(json.lineJoin, (v) =>
      stroke.setLineJoin(v)
    );
    doif(json.miterLimit, (v) =>
      stroke.setMiterLimit(v)
    );
    doif(json.width, (v) =>
      stroke.setWidth(v)
    );
  ...
  private deserializeColor(
    ...
    if (fill.gradient) {
      ...
      if (
        0 === type.indexOf("linear(")
      ) {
        gradient =
          this.deserializeLinearGradient(
            fill.gradient
          );
      } else if (
        0 === type.indexOf("radial(")
      ) {
        gradient =
          this.deserializeRadialGradient(
            fill.gradient
          );
      } else
  ...
  private deserializeLinearGradient(
    ...
    let rx = /\w+\((.*)\)/m;
    let [x0, y0, x1, y1] = JSON.parse(
      json.type.replace(rx, "[$1]")
    );
