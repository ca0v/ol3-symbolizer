...
let fromJson = (() => {
  let fromJsonConverter =
    new StyleConverter();
  return (
    style: ArcGisFeatureServerLayer.Symbol
  ) =>
    fromJsonConverter.fromJson(style);
})();
...
describe("esriSMS Tests", () => {
  it("esriSMSCircle", () => {
    ...
    shouldEqual(
      circleJson.circle.fill.color,
      rgba(baseline.color),
      "circleJson fill color"
    );
    shouldEqual(
      circleJson.circle.fill.pattern,
      null,
      "circleJson fill pattern is solid"
    );

    shouldEqual(
      circleJson.circle.stroke.color,
      rgba(baseline.outline.color),
      "circleJson stroke color"
    );
    shouldEqual(
      circleJson.circle.stroke.width,
      (baseline.outline.width * 4) / 3,
      "circleJson stroke width"
    );
    shouldEqual(
      circleJson.circle.stroke.lineCap,
      undefined,
      "circleJson stroke lineCap"
    );
    shouldEqual(
      circleJson.circle.stroke.lineDash,
      undefined,
      "circleJson stroke lineDash"
    );
    shouldEqual(
      circleJson.circle.stroke.lineJoin,
      undefined,
      "circleJson stroke lineJoin"
    );
  });
  ...
  it("esriSMSCross", () => {
    ...
    shouldEqual(
      ...
      22.62741699796952,
      "size"
    ); // no idea why, but let me know if it changes :)
  });
});
